# bd1-redsocial-grupo7
# Red Social Estudiantil - Pascualina

## Nombre del proyecto

Modelo conceptual de base de datos para una red social estudiantil.

## Integrantes
- Luis Mateo Cataño Rivera

## Descripción del caso

El proyecto consiste en diseñar el modelo conceptual de una base de datos para una red social estudiantil.

La plataforma busca facilitar la comunicación y la interacción entre estudiantes mediante perfiles, publicaciones, comentarios, conexiones, grupos, eventos y mensajes.

## Objetivo

Diseñar un Modelo Entidad-Relación que permita representar las principales entidades, atributos, relaciones y cardinalidades necesarias para el funcionamiento de la plataforma.

## Entidades principales

- ESTUDIANTE
- PUBLICACIÓN
- COMENTARIO
- GRUPO
- MIEMBRO_GRUPO
- EVENTO
- ASISTENCIA_EVENTO
- MENSAJE

## Relaciones principales

- ESTUDIANTE CREA PUBLICACIÓN.
- ESTUDIANTE REALIZA COMENTARIO.
- PUBLICACIÓN RECIBE COMENTARIO.
- ESTUDIANTE REGISTRA MIEMBRO_GRUPO.
- GRUPO TIENE MIEMBRO_GRUPO.
- ESTUDIANTE PARTICIPA EN EVENTO.
- ESTUDIANTE REGISTRA ASISTENCIA_EVENTO.
- EVENTO TIENE ASISTENCIA_EVENTO.
- ESTUDIANTE ENVÍA MENSAJE.
- ESTUDIANTE RECIBE MENSAJE.
- ESTUDIANTE SE CONECTA CON ESTUDIANTE.
